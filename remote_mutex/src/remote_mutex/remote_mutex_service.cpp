 ...
 public:
  ...
  explicit RemoteMutexService(const char* name)
    ...
    service = ns.advertiseService(
      ...
      this);
    ros::param::get("/topic", root_topic_);
    root_topic_ = root_topic_ + "_state";
    ROS_ERROR("hello%s",root_topic_.c_str());
    // ns.param<std::string>( "/topic", root_topic_, "THEN_0_1_015_state");
    ns.param<int>( "/enum_robot", enum_robot_, 1);
    state_subscriber_ = ns.subscribe(root_topic_.c_str(), 1000, &RemoteMutexService::RootStateCallback, this );
  ...
  bool MutexRequest(remote_mutex::remote_mutex_msg::Request &req,
    ...
    if (req.request) {
      ...
      if (locked) {
        ...
        else if( is_eq(GetBitmask(req.name), top_level_state_.highest) && (top_level_state_.highest.type == 6) )
